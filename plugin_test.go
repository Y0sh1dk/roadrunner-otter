...
func TestPlugin_Init_DefaultsAppliedPerEntry(t *testing.T) {
	...
	assert.Equal(t, defaultMaxBodyBytes, p.config.Paths[0].MaxBodyBytes)
	assert.Equal(t, defaultCacheMaxEntries, p.config.Paths[0].CacheMaxEntries)
}
...
func TestPlugin_Init_AppliesPerPathConfig(t *testing.T) {
	...
	assert.Equal(t, int64(1024), got.MaxBodyBytes)
...
func TestConfig_Validate(t *testing.T) {
	...
	c = &Config{Paths: []PathConfig{{Pattern: "^/x", Methods: []string{""}, CacheTTL: testTTL}}}
	c.InitDefaults()
	err = c.Validate()
	require.Error(t, err)
