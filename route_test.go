...
func TestRouteTable_NoMatchBypasses(t *testing.T) {
	...
	rt := mustBuild(t, &Config{
		Paths: []PathConfig{{Pattern: "^/api/", Methods: safeMethods(), CacheTTL: routeTestTTL}},
	})
...
func TestRouteTable_DisabledEntryBypasses(t *testing.T) {
	...
	rt := mustBuild(t, &Config{
		Paths: []PathConfig{
			...
			{Pattern: "^/api/", Methods: safeMethods(), CacheTTL: routeTestTTL},
		},
	})
...
func TestRouteTable_FirstMatchWins(t *testing.T) {
	...
	rt := mustBuild(t, &Config{
		Paths: []PathConfig{
			...
			{Pattern: "^/api/.*", Methods: safeMethods(), CacheTTL: routeTestTTL}, // would also match /api/health
		},
	})
...
// minimum required fields (pattern + methods) still gets the max_body_bytes
// default (the only field that's defaulted).
func TestRouteTable_PerEntryDefaultsApplied(t *testing.T) {
	t.Parallel()

	rt := mustBuild(t, &Config{
		Paths: []PathConfig{{Pattern: "^/.*", Methods: []string{"GET"}, CacheTTL: routeTestTTL}},
	})
...
func TestRouteTable_BadPatternErrors(t *testing.T) {
	...
	cfg := &Config{Paths: []PathConfig{{Pattern: "[", Methods: safeMethods(), CacheTTL: routeTestTTL}}}
	cfg.InitDefaults()
...
func TestConfig_Validate_PathRequirements(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			cfg:  Config{Paths: []PathConfig{{Pattern: "^/x", Methods: safeMethods(), MaxBodyBytes: -1}}},
			want: "max_body_bytes",
		},
		{
		...
		{
			name: "missing cache_ttl on active entry",
			cfg:  Config{Paths: []PathConfig{{Pattern: "^/x", Methods: safeMethods()}}},
			want: "cache_ttl is required",
		},
		{
			name: "negative cache_max_entries",
			cfg: Config{Paths: []PathConfig{{
				Pattern: "^/x", Methods: safeMethods(),
				CacheTTL: routeTestTTL, CacheMaxEntries: -1,
			}}},
			want: "cache_max_entries",
		},
		{
			name: "empty cache_statuses (explicitly set to empty)",
			cfg: Config{Paths: []PathConfig{{
				Pattern: "^/x", Methods: safeMethods(),
				CacheTTL: routeTestTTL, CacheStatuses: []int{},
			}}},
			want: "cache_statuses",
		},
		{
			name: "invalid HTTP status in cache_statuses",
			cfg: Config{Paths: []PathConfig{{
				Pattern: "^/x", Methods: safeMethods(),
				CacheTTL: routeTestTTL, CacheStatuses: []int{200, 9999},
			}}},
